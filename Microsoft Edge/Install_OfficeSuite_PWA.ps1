function New-Shortcut {
    ...
    begin {
        if ($IconURL) {
            ...
            Invoke-WebRequest -Uri $IconURL -OutFile "$IconPath\$IconName" -UseBasicParsing            # Set the IconLocation to the downloaded file
...
#Make a hashtable that has the shortcut names and the links
$Linkhash = @{
...
#Make a hashtable that has the name and the icon URL
$IconHash = @{
    "Outlook"    = "https://sapwaicons.blob.core.windows.net/icons/outlook.ico"
    "Teams"      = "https://sapwaicons.blob.core.windows.net/icons/teams.ico"
    "Word"       = "https://sapwaicons.blob.core.windows.net/icons/word.ico"
    "Excel"      = "https://sapwaicons.blob.core.windows.net/icons/excel.ico"
    "PowerPoint" = "https://sapwaicons.blob.core.windows.net/icons/powerpoint.ico"
}


#Iterate through each program and create a shortcut
foreach ($program in $Linkhash.Keys) {
    $props = @{
        'ShortcutPath' = Join-Path -Path ([Environment]::GetFolderPath("CommonDesktopDirectory")) -ChildPath "$program.lnk"
        'TargetPath'   = "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" 
        'Arguments'    = "-profile-directory=Default -app=$($Linkhash[$program])"
        'IconName'     = "$program.ico"
        'IconURL'      = $IconHash[$program]
        'IconPath'     = "C:\Temp"
    }
    New-Shortcut @props
}
